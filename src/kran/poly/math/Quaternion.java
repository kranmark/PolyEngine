...
/**
 * <code>Quaternion</code> defines a single example of a more general class of hypercomplex numbers.
 * Quaternions extends a rotation in three dimensions to a rotation in four dimensions. This avoids
 * "gimbal lock" and allows for smooth continuous rotation.
 * 
...
public final class Quaternion implements Cloneable, java.io.Serializable {
	...
	/**
	 * Constructor instantiates a new <code>Quaternion</code> object initializing all values to
	 * zero, except w which is initialized to 1.
	 * 
	 */
	public Quaternion() {
		x = 0;
	...
	/**
	 * Constructor instantiates a new <code>Quaternion</code> object from the given list of
	 * parameters.
	 * 
	 * @param x
	...
	/**
	 * sets the data in a <code>Quaternion</code> object from the given list of parameters.
	 * 
	 * @param x
	 *             the x value of the quaternion.
	 * @param y
	...
	/**
	 * Sets the data in this <code>Quaternion</code> object to be equal to the passed
	 * <code>Quaternion</code> object. The values are copied producing a new object.
	 * 
	 * @param q
	...
	/**
	 * Constructor instantiates a new <code>Quaternion</code> object from a collection of rotation
	 * angles.
	 * 
	 * @param angles
	 *             the angles of rotation (x, y, z) that will define the <code>Quaternion</code>.
	 */
	...
	/**
	 * Constructor instantiates a new <code>Quaternion</code> object from an interpolation between
	...
	/**
	 * Constructor instantiates a new <code>Quaternion</code> object from an existing quaternion,
	 * creating a copy.
	 * 
	 * @param q
	...
	/**
	 * <code>fromAngles</code> builds a Quaternion from the Euler rotation angles (x,y,z) aka
	 * (pitch, yaw, roll)). Note that we are applying in order: (y, z, x) aka (yaw, roll, pitch)
	 ...
	 * 
	 * @param xAngle
	 *             the Euler pitch of rotation (in radians). (aka Attitude, often rot around x)
	 * @param yAngle
	 *             the Euler yaw of rotation (in radians). (aka Heading, often rot around y)
	 * @param zAngle
	 *             the Euler roll of rotation (in radians). (aka Bank, often rot around z)
	 */
	 ...
	 * 
	 * @param angles
	 *             the float[] in which the angles should be stored, or null if you want a new
	 *             float[] to be created
	...
	/**
	 * 
	 * <code>fromRotationMatrix</code> generates a quaternion from a supplied matrix. This matrix
	 * is assumed to be a rotational matrix.
	 * 
	 * @param matrix
	...
	/**
	 * <code>toRotationMatrix</code> converts this quaternion to a rotational matrix. Note: the
	 * result is created from a normalized version of this quat.
	 * 
	...
	/**
	 * <code>toRotationMatrix</code> converts this quaternion to a rotational matrix. The result is
	 * stored in result. 4th row and 4th column values are untouched. Note: the result is created
	 * from a normalized version of this quat.
	 * 
	 * @param result
	 ...
	 * column is returned as a <code>Vector3f</code> object.
	 * 
	 * @param i
	...
	/**
	 * <code>fromAngleAxis</code> sets this quaternion to the values specified by an angle and an
	 * axis of rotation. This method creates an object, so use fromAngleNormalAxis if your axis is
	 * already normalized.
	 * 
	 * @param angle
	...
	/**
	 * <code>fromAngleNormalAxis</code> sets this quaternion to the values specified by an angle
	 * and a normalized axis of rotation.
	 * 
	 * @param angle
	 *             the angle to rotate (in radians).
	 * @param axis
	 ...
	 * quaternion. The values are stored as follows: The axis is provided as a parameter and built
	...
	/**
	 * Sets the values of this quaternion to the slerp (spherical linear interpolation) from itself
	 * to q2 by changeAmnt
	 * 
	 * @param q2
	...
	/**
	 * Sets the values of this quaternion to the nlerp (normalized linear interpolation) from
	 * itself to q2 by blend.
	 * 
	 * @param q2
	 * @param blend
	 */
	...
	/**
	 * <code>add</code> adds the values of this quaternion to those of the parameter quaternion.
	...
	/**
	 * <code>subtract</code> subtracts the values of the parameter quaternion from those of this
	 ...
	 * commutative so q * p != p * q.
	 * 
	 * It IS safe for q and res to be the same object. It IS safe for this and res to be the same
	 * object.
	 * 
	 * @param q
	 *             the quaternion to multiply this quaternion by.
	...
	/**
	 * <code>apply</code> multiplies this quaternion by a parameter matrix internally.
	 * 
	 * @param matrix
	...
	/**
	 * 
	 * <code>fromAxes</code> creates a <code>Quaternion</code> that represents the coordinate
	 * system defined by three axes. These axes are assumed to be orthogonal and no error checking
	 * is applied. Thus, the user must insure that the three axes being provided indeed represents
	 * a proper right handed coordinate system.
	 * 
	 * @param axis
	...
	/**
	 * 
	 * <code>toAxes</code> takes in an array of three vectors. Each vector corresponds to an axis
	 * of the coordinate system defined by the quaternion rotation.
	 * 
	 * @param axis
	...
	/**
	 * <code>mult</code> multiplies this quaternion by a parameter vector. The result is returned
	 * as a new vector.
	 * 
	 * @param v
	...
	/**
	 * Multiplies this Quaternion by the supplied quaternion. The result is stored in this
	 * Quaternion, which is also returned for chaining. Similar to this *= q.
	 * 
	 * @param q
	 ...
	 *             the vector to multiply this quaternion by.
	 * @param store
	 *             the vector to store the result in. It IS safe for v and store to be the same
	 *             object.
	...
	/**
	 * <code>mult</code> multiplies this quaternion by a parameter scalar. The result is returned
	 * as a new quaternion.
	 * 
	 * @param scalar
	 *             the quaternion to multiply this quaternion by.
	...
	/**
	 * <code>mult</code> multiplies this quaternion by a parameter scalar. The result is stored
	 * locally.
	 * 
	 * @param scalar
	 *             the quaternion to multiply this quaternion by.
	...
	/**
	 * <code>dot</code> calculates and returns the dot product of this quaternion with that of the
	...
	/**
	 * <code>normalize</code> normalizes the current <code>Quaternion</code>. The result is stored
	 * internally.
	 */
	...
	/**
	 * <code>inverse</code> returns the inverse of this quaternion as a new quaternion. If this
	 * quaternion does not have an inverse (if its normal is 0 or less), then null is returned.
	 * 
	...
	/**
	 * <code>inverse</code> calculates the inverse of this quaternion and returns this quaternion
	 * after it is calculated. If this quaternion does not have an inverse (if it's normal is 0 or
	...
	/**
	 * 
	 * <code>toString</code> creates the string representation of this <code>Quaternion</code>. The
	 * values of the quaternion are displaced (x, y, z, w), in the following manner: <br>
	 * (x, y, z, w)
	 * 
	...
	/**
	 * <code>equals</code> determines if two quaternions are logically equal, that is, if the
	 * values of (x, y, z, w) are the same for both quaternions.
	 * 
	 * @param o
	...
	/**
	 * 
	 * <code>hashCode</code> returns the hash code value as an integer and is supported for the
	 * benefit of hashing based collection classes such as Hashtable, HashMap, HashSet etc.
	 * 
	...
	/**
	 * <code>readExternal</code> builds a quaternion from an <code>ObjectInput</code> object. <br>
	...
	/**
	 * <code>lookAt</code> is a convenience method for auto-setting the quaternion based on a
	...
	public void lookAt(Vector3f direction, Vector3f up) {
		...
		vars.vec3f3.set(direction).normalizeLocal();
		vars.vec3f1.set(up).crossLocal(direction).normalizeLocal();
		vars.vec3f2.set(direction).crossLocal(vars.vec3f1).normalizeLocal();
		fromAxes(vars.vec3f1, vars.vec3f2, vars.vec3f3);
		vars.release();
	}
	
	/**
	 * @return A new quaternion that describes a rotation that would point you in the exact
	 *         opposite direction of this Quaternion.
	 */
	...
	/**
	 * FIX ME: This seems to have singularity type issues with angle == 0, possibly others such as
	 * PI.
	 * 
	 * @param store
	 *             A Quaternion to store our result in. If null, a new one is created.
	 * @return The store quaternion (or a new Quaterion, if store is null) that describes a
	 *         rotation that would point you in the exact opposite direction of this Quaternion.
	 */
	...
	/**
	 * @return This Quaternion, altered to describe a rotation that would point you in the exact
	 *         opposite direction of where it is pointing currently.
	 */
