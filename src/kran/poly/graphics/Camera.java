...
public class Camera {
	...
	public boolean isRefreshViewMatrix() {
		return refreshViewMatrix;
	}
	
	public boolean isRefreshInverseProjectionViewMatrix() {
		return refreshInverseProjectionViewMatrix;
	}
	
	public boolean isRefreshProjectionMatrix() {
		return refreshProjectionMatrix;
	}
	...
	private void doRefreshInverseProjectionViewMatrix() {
		...
		invViewProjectionMatrix.multLocal(getViewMatrix());
		invViewProjectionMatrix.invertLocal();
	...
	public void getEyeRay(float x, float y, Vector3f res) {
		...
		getInverseProjectionViewMatrix().mult(tmp3, tmp3);
		tmp3.multLocal(1.0f / tmp3.w);
		tmp0.set(tmp3.x, tmp3.y, tmp3.z);
		res.set(tmp0);
		res.subtractLocal(position);
	}
	...
	public void setLookAt(Vector3f position, Vector3f lookAt, Vector3f up) {
		...
		tmp0.normalizeLocal();
		tmp1.set(lookAt);
		tmp1.subtractLocal(position);
		tmp1.normalizeLocal();
		setDirection(tmp1);
		/* right = direction x up */
		tmp1.cross(tmp0, right);
	...
	private void doRefreshViewMatrix() {
		...
		tmp1.normalizeLocal();
		/* right = direction x up */
		tmp0.cross(tmp1, tmp1);
		/* up = right x direction */
		tmp1.cross(tmp0, tmp2);
